...
deploy: package
	...
	curl -T *.deb -ualdrinleal:$(BINTRAY_APIKEY) https://api.bintray.com/content/ingenieux/lfs-deb/$(NAME)/$(VERSION)/$(shell basename *.deb)
	curl -T *.rpm -ualdrinleal:$(BINTRAY_APIKEY) https://api.bintray.com/content/ingenieux/lfs-rpm/$(NAME)/$(VERSION)/$(shell basename *.rpm)
